...
const CreateProject = async (req, res) => {
    try {
        ...
        let coverUrl = req.files && req.files.cover ? await uploadImageToImgBB(req.files.cover[0].path) : null;
        let screenshotUrl = req.files && req.files.screenshot ? await uploadImageToImgBB(req.files.screenshot[0].path) : null;
...
const updateSpecificProject = async (req, res) => {
    try {
        let coverUrl = req.files && req.files.cover ? await uploadImageToImgBB(req.files.cover[0].path) : req.body.cover;
        let screenshotUrl = req.files && req.files.screenshot ? await uploadImageToImgBB(req.files.screenshot[0].path) : req.body.screenshot;
...
const activateSingleProject = async (req, res) => {
    try {
        ...
        const statusActivation = await ProjectModel.findByIdAndUpdate(specificProjectId, { status: activeStatus }, { new: true });
        if (statusActivation) {
            return res.json({ message: "This project has been activated", data: statusActivation });
        }
...
const deactivateSingleProject = async (req, res) => {
    try {
        ...
        const statusDeactivation = await ProjectModel.findByIdAndUpdate(specificProjectId, { status: inactiveStatus }, { new: true });
        if (statusDeactivation) {
            return res.json({ message: "This project has been deactivated", data: statusDeactivation });
        }
